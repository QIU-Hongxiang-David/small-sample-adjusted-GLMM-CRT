...
run.all.methods<-function(data,outcome.type=c("binary","count"),true.model,alpha=.05){
    ...
    formula<-if(true.model=="A"){
        ...
        Y~treatment+individual.X1+individual.X2+individual.X3+(1|cluster.id)
    }else if(true.model=="C"){
        Y~treatment+cluster.X+(1|cluster.id)
    }else if(true.model=="D"){
        Y~treatment+individual.X1+individual.X2+individual.X3+cluster.X+(1|cluster.id)
    }
    ...
    output<-rbind(output,tryCatch({
        ...
        df.containment<-nrow(data)-n_distinct(data$cluster.id)-ifelse(true.model %in% c("B","D"),3,0)
        df.BW<-n_distinct(data$cluster.id)-length(all.vars(formula))+1
        df.IO<-df.BW+ifelse(true.model %in% c("B","D"),3,0)
    ...
    output<-rbind(output,tryCatch({
        ...
        df.containment<-nrow(data)-n_distinct(data$cluster.id)-ifelse(true.model %in% c("B","D"),4,0)
        df.BW<-n_distinct(data$cluster.id)-length(all.vars(formula.model2))+1
        df.IO<-df.BW+ifelse(true.model %in% c("B","D"),4,1)
...
done.files<-list.files("cache/")
run.once<-function(i,...){
    if(paste0("sim",i,".csv") %in% done.files){
        return(NULL)
    }
...
n_cores<-detectCores(logical=FALSE) #=4 on the machine on which we ran the simulation
