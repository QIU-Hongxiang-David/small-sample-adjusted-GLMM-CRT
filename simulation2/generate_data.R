...
gen.individual.covariate1<-function(n) rnorm(n)
gen.individual.covariate2<-function(n) rnorm(n)
gen.individual.covariate3<-function(n) rbinom(n,1,.5)
...
calc.Poisson.random.effect.var<-function(ICC,true.model,max.sigma2=20,...){
    #mean rate is E[Y]=e^(beta.individual1^2/2+beta.individual2^2/2+sigma^2/2)*E[e^(beta.individual3*indivicual.X3)]*E[e^(beta.cluster*cluster.X)] where sigma^2 is random effect variance
    ...
    uniroot(function(sigma2){
        ...
        if(true.model %in% c("B","D")){
            denominator<-denominator*exp(beta.individual1^2/2+beta.individual2^2/2)*.5*(1+exp(beta.individual3))
        }
...
generate.data<-function(n.cluster,mean.size,coef.var,ICC,outcome.type=c("binary","count"),true.model,max.sigma2=20,...){
    ...
    lapply(1:n.cluster,function(i){
        ...
        output<-data.frame(cluster.id=i,individual.id=1:ni,
                           ...
                           individual.X1=gen.individual.covariate1(ni),
                           individual.X2=gen.individual.covariate2(ni),
                           individual.X3=gen.individual.covariate3(ni),
                           useless.cluster.X=gen.cluster.covariate(1),
                           useless.individual.X=gen.individual.covariate1(ni),
                           treatment=treatments[i])%>%
            mutate(linear.part=rnorm(1,sd=sqrt(random.effect.var)))
        if(true.model %in% c("B","D")){
            output<-output%>%mutate(linear.part=linear.part+beta.individual1*individual.X1+beta.individual2*individual.X2+beta.individual3*individual.X3)
        }
        if(true.model %in% c("C","D")){
